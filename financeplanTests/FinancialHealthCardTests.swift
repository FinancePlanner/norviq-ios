...
final class FinancialHealthCardTests: XCTestCase {
  func testDashboardInsightsResponseDecodingIncludesFinancialHealth() async throws {
    await Task.yield()
    let payload = """
    {
      "savingsRate": 22.5,
  ...
  func testFinancialHealthCardStateCoversLoadingLoadedAndUnavailable() async {
    await Task.yield()
