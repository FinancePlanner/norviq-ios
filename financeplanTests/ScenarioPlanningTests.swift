...
final class ScenarioPlanningTests: XCTestCase {
  private final class ServiceMock: ScenarioPlanningServiceProtocol, @unchecked Sendable {
    ...
    func portfolios() async throws -> [ScenarioPortfolio] {
      portfoliosValue
    }

    func goals() async throws -> [ScenarioGoal] {
      goalsValue
    }
    ...
    func riskProfiles() async throws -> [ScenarioRiskProfile] {
      riskProfilesValue
    }
    ...
    func saveRiskProfile(
      ...
      async throws -> ScenarioRiskProfile
    {
      .init(
        id: UUID(),
        holdingId: holdingID,
    ...
    func createRun(scenarioID: UUID, snapshotID: UUID, seed: Int64?) async throws -> ScenarioRunSummary {
      createdScenarioID = scenarioID; return try XCTUnwrap(
        createdRun
      ) }
    ...
    func run(id: UUID) async throws -> ScenarioRunSummary {
      try XCTUnwrap(polledRun)
    }
  ...
  func testResultDecodesBackendSnakeCase() async throws {
    await Task.yield()
    let data = Data(
      #"{"id":"00000000-0000-0000-0000-000000000001","state":"completed","progress":1,"engineVersion":"scenario-engine-v1","errorMessage":null,"result":{"timeline":[{"elapsed_months":12,"value":125000}],"maximum_drawdown":0.18,"goal_probability":0.72,"expected_shortfall":5000,"ending_value":78000,"portfolio_change_percent":-0.22,"goal_delay_months":14,"required_monthly_contribution":680,"contribution_delta":180,"recovery_months":18,"expense_impact_monthly":180}}"#.utf8
    )
  ...
  func testResultDecodesPercentileFan() async throws {
    await Task.yield()
    let data = Data(
      #"{"id":"00000000-0000-0000-0000-000000000001","state":"completed","progress":1,"engineVersion":"scenario-engine-v1","errorMessage":null,"result":{"percentile_bands":[{"elapsed_months":12,"p10":90,"p25":100,"p50":120,"p75":130,"p90":150}],"maximum_drawdown":0.2}}"#.utf8
    )
    let run = try JSONDecoder().decode(ScenarioRunSummary.self, from: data)
  ...
  func testViewModelLoadsWorkspaceAndPollsActiveRun() async {
    ...
    service.goalsValue = [
      .init(
        ...
        annualContributionGrowth: nil,
        inflationAssumption: nil
      )
    ]
    service.runsValue = [
      .init(id: id, state: "running", progress: 0.25, engineVersion: "v1", errorMessage: nil, result: nil)
    ]
    service.polledRun = .init(
      id: id,
      state: "completed",
      progress: 1,
      engineVersion: "v1",
  ...
  func testSubmitPrependsQueuedRun() async {
    let service = ServiceMock()
  ...
  func testSavedScenarioRequiresSnapshotReviewAndCanRunAgain() async {
    let service = ServiceMock()
  ...
  func testPrivatePDFReportIsReadable() async throws {
    await Task.yield()
  ...
  func testMultiAssetAssumptionsDecodeAndValidate() async throws {
    await Task.yield()
