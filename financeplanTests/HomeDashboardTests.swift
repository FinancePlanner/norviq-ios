...
final class HomeDashboardTests: XCTestCase {
  ...
  @MainActor
  func testEveryHomeTabIsReachable() async {
    await Task.yield()
  ...
  @MainActor
  func testMoreMenuIncludesMarketsTab() async {
    await Task.yield()
    XCTAssertTrue(
  ...
  func testHomeScreenRegistersPrimaryTabsBeforeOverflow() async throws {
    await Task.yield()
  ...
  @MainActor
  func testTabListsDoNotOverlapOrDuplicate() async {
    await Task.yield()
    XCTAssertEqual(
  ...
  func testOverflowTabsDoNotNestNavigationStacks() async throws {
    await Task.yield()
    let home = try String(
