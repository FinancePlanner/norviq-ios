...
final class ExpensesLogicTests: XCTestCase {
  ...
  func testBudgetMonthSummaryCalculations() async throws {
    await Task.yield()
    let calendar = Calendar(identifier: .gregorian)
    let monthStart = try XCTUnwrap(calendar.date(from: DateComponents(year: 2024, month: 4, day: 1)))
  ...
  func testBudgetPlanItemInitialization() async {
    await Task.yield()
    let id = UUID()
  ...
  func testBudgetActivityInitialization() async {
    await Task.yield()
    let id = UUID()
    let date = Date()
