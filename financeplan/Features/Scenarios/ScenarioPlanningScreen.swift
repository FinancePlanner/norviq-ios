...
@MainActor @Observable final class ScenarioPlanningViewModel {
  ...
  func submit(_ request: ScenarioRunRequest) async {
    isSubmitting = true
    defer { isSubmitting = false }
    do {
      runs.insert(try await service.createRun(request), at: 0)
    } catch {
      errorMessage = "The scenario could not be queued."
    }
  }
  ...
  private var comparisonChart: some View {
    ...
    return Chart {
      ForEach(filteredRuns) { run in
        ...
        ForEach(run.result?.timeline ?? []) { point in
          LineMark(
