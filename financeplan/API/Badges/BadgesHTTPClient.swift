...
struct BadgesHTTPClient: Sendable {
    ...
    init(baseURL: URL, session: any HTTPClientSession = URLSession.shared, authTokenProvider: @escaping @Sendable () -> String? = { nil }) {
        self.client = BaseHTTPClient(
            ...
            logger: Logger(subsystem: Bundle.main.bundleIdentifier ?? "financeplan", category: "BadgesHTTPClient"),
            decoder: .stockPlanShared
        )
    }

    func getBadges() async throws -> BadgesListResponse {
        try await client.call(GetBadgesEndpoint(), errorType: Error.self)
    }
}
