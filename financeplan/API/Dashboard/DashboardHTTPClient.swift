...
nonisolated struct DashboardHTTPClient: Sendable {
    ...
    init(baseURL: URL, session: any HTTPClientSession = URLSession.shared, authTokenProvider: @escaping @Sendable () async -> String? = { nil }) {
        self.client = BaseHTTPClient(
            ...
            logger: Logger(subsystem: Bundle.main.bundleIdentifier ?? "financeplan", category: "DashboardHTTPClient"),
            decoder: .stockPlanShared
        )
    }

    func getDashboard() async throws -> DashboardResponse {
        try await client.call(GetDashboardEndpoint(), errorType: Error.self)
    }
